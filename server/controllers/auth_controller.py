...
from helpers import jwt_helper, response_helper, key_helper
from models import auth_model


def register(data):
    if not key_helper(data, {"username": str, "password": str}):
    ...
    if res[0] == 0:
        if res[1] == 409:
            return (
                0,
                response_helper.response(409, -1, {"app_message": "Username is already taken"}),
            )
...
def login(data):
    ...
    if res[0] == 0:
        if res[1] == 401:
            return (
                0,
                response_helper.response(401, -1, {"app_message": "Invalid username or password"}),
            )
    else:
        token = jwt_helper.sign(
            {"id": res[1][0], "username": res[1][1], "exp": int(time.time() + 24 * 60 * 60)}
        )
        return (
            1,
            response_helper.response(
                200, -1, {"app_message": "Login successfully", "token": token}
            ),
        )
